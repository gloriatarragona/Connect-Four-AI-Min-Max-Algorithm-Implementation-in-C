...
return 0;
 ...
 int ordinadorAhuma(int col){ //Inversa de la funci� anterior
 return col+1;
 }
...
void ferTirada(char tauler[N][N],int col, int jugador){ //Fa la tirada d'un jugador o del ordinador
    ...
    if(columnaPlena(tauler,col)){ //Cas 2: Introduim una columna que est� plena (Error)
...
void trobarTauler(Node*p){ //Busca quina �s la millor tirada despres de fer el Minimax
    ...
    copiaTauler(p->tauler,aux[rand()%(opcions)]->tauler); //Si dos taulers tenen la mateixa puntuaci� tria un aleatoriament
    return ;
}
...
int funcioHeuristica(char tauler[N][N]){ //Calcula els valors dels nodes
    ...
    for(int i=0;i<N;i++){ //Mira els diferents subconjunts de 4 on nom�s un dels jugadors te fitxes
        for(int j=0;j<N;j++){
        ...
        if(j<N-3){
            ...
            for(int k=0;k<4;k++){
            if(tauler[i][j+k]=='1'){
                    valor=valor-profunditat; //Resta una quantitat per cada fitxa que t� la persona
            }
            }
            }
            ...
            for(int k=0;k<4;k++){
            if(tauler[i][j+k]=='2'){
                    valor=valor+(profunditat/2); //Suma una quantita per cada fitxa que t� l'ordinador
            }
            }
            }
            }
...
void menu(void){ //Printeja el men� de benvinguda
    for(int i=0;i<50;i++){
    printf("+");
    }
    for(int i=0;i<15;i++){
...
void exitMenuGuanyador(void){ //Printeja el men� de si guanya l'hum�
    printf("\n");
     for(int i=0;i<50;i++){
    printf("+");
    }
...
void exitMenuPerdedor(void){ //printeja el men� de si perd l'hum�
    printf("\n");
     for(int i=0;i<50;i++){
    printf("+");
    }
...
void exitMenuEmpat(void){ //Printeja el men� d'empat
    printf("\n");
     for(int i=0;i<50;i++){
    printf("+");
    }
...
Node *arrel=malloc(sizeof(Node));
    ...
    int player;
    int col;
    for(int i=0;i<(N*N);i++){ //Triem el N*N doncs �s el nombre m�xim de movimients que es poden fer en una partida
        if(i>6){ //Comprova el 4 en linea nom�s si hi ha ja 4 fitxes d'un jugador al tauler
